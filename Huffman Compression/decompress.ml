...
(* Decompression*)
let decompress fileIn fileOut =
  let trie_ref = ref [] in
    loadbit fileIn "tmp_203049238281.txt" trie_ref;
    let channel_in = open_in "tmp_203049238281.txt" in
    let channel_out = open_out fileOut in
      code_to_str !trie_ref channel_in channel_out;;
